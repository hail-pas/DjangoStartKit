...
class ConnectManageConsumer(AsyncUJsonWebsocketConsumer):
    ...
    async def post_accept(self):
        # 设置在线
        await AsyncRedisUtil.r.setbit(keys.RedisCacheKey.ProfileOnlineKey.format(profile_id=self.profile.id),
                                      self.profile.id, 1)
        # 添加连接信息，防止重复连接
        ...
        # 加入系统群组
        await self.channel_layer.group_add(defines.ChatContextFormatKey.SystemCenter, self.channel_name)
        # 加入用户的群组
        for group_id in await AsyncRedisUtil.r.smembers(
            ...
            await self.channel_layer.group_add(defines.ChatContextFormatKey.Group % group_id, self.channel_name)
    ...
    async def disconnect(self, close_code):
        if close_code not in [defines.ServiceCode.Unauthorized.value, defines.ServiceCode.DeviceRestrict.value]:
            ...
            # 离开系统群组
            await self.channel_layer.group_discard(defines.ChatContextFormatKey.SystemCenter, self.channel_name)

            # 离开用户群组
            for group_id in await AsyncRedisUtil.r.smembers(
                keys.RedisCacheKey.ProfileGroupSet.format(profile_id=self.profile.id), encoding="utf-8"
            ):
                await self.channel_layer.group_discard(defines.ChatContextFormatKey.Group % group_id, self.channel_name)
            # 删除连接信息
...
class ReplyMixin(ConnectManageConsumer, ABC):
    ...
    async def send_error(self, code: defines.ServiceCode, message: Optional[str]):
        await self.send(
            bytes_data=self.gen_reply_bytes(
                context=defines.ChatContextFormatKey.SystemCenter,
                chat_type=defines.ChatType.SystemCenter.value,
                sender_info=defines.SystemInfo,
                type_=defines.MessageType.Error,
    ...
    async def send_text(self, context, chat_type, sender_info, text: str):
        await self.send(
            bytes_data=self.gen_reply_bytes(
                context, chat_type, sender_info, defines.MessageType.Text, self.gen_text_payload(text)
            )
        )
    ...
    async def send_file(
        ...
        await self.send(
            bytes_data=self.gen_reply_bytes(
                ...
                type_,
                defines.PayLoadFile(
                    code=defines.ServiceCode.Success, id=id_, url=url, name=name, size=size, extension=extension
                ),
            )
        )
    ...
    async def send_online(self, user_info: defines.BaseIdentifierInfo):
        await self.send(
            bytes_data=self.gen_reply_bytes(
                ...
                defines.SystemInfo,
                defines.MessageType.Online,
                defines.PayloadOnline(code=defines.ServiceCode.Success, user_info=user_info),
            )
        )
    ...
    async def send_offline(self, user_info: defines.BaseIdentifierInfo):
        await self.send(
            bytes_data=self.gen_reply_bytes(
                ...
                defines.ChatType.SystemCenter.value,
                defines.SystemInfo,
                defines.MessageType.Offline,
                defines.PayloadOffline(code=defines.ServiceCode.Success, user_info=user_info),
            )
        )
    ...
    async def send_join(self, user_info: defines.BaseIdentifierInfo):
        await self.send(
            bytes_data=self.gen_reply_bytes(
                ...
                defines.ChatType.SystemCenter.value,
                defines.SystemInfo,
                defines.MessageType.Join,
                defines.PayloadJoin(code=defines.ServiceCode.Success, user_info=user_info),
            )
        )
    ...
    async def send_typing(self, user_info: defines.BaseIdentifierInfo):
        await self.send(
            bytes_data=self.gen_reply_bytes(
                ...
                defines.ChatType.SystemCenter.value,
                defines.SystemInfo,
                defines.MessageType.Typing,
                defines.PayloadTyping(code=defines.ServiceCode.Success, user_info=user_info),
            )
        )
    ...
    async def send_stop_typing(self, user_info: defines.BaseIdentifierInfo):
        await self.send(
            bytes_data=self.gen_reply_bytes(
                ...
                defines.ChatType.SystemCenter.value,
                defines.SystemInfo,
                defines.MessageType.StopTyping,
                defines.PayloadStopTyping(code=defines.ServiceCode.Success, user_info=user_info),
            )
        )
    ...
    async def send_message_read(self, message_id: int, user_info: defines.BaseIdentifierInfo):
        await self.send(
            bytes_data=self.gen_reply_bytes(
                ...
                defines.ChatType.SystemCenter.value,
                defines.SystemInfo,
                defines.MessageType.MessageRead,
                defines.PayloadMessageRead(
                    code=defines.ServiceCode.Success, message_id=message_id, user_info=user_info
                ),
            )
        )
    ...
    async def send_message_sent(self, message_id: int):
        await self.send(
            bytes_data=self.gen_reply_bytes(
                ...
                defines.ChatType.SystemCenter.value,
                defines.SystemInfo,
                defines.MessageType.MessageSent,
                defines.PayloadMessageSent(code=defines.ServiceCode.Success, message_id=message_id, ),
            )
        )
    ...
    async def send_new_un_read_count(self, connection_info: defines.ConnectionInfo, count: int, message_id: int):
        await self.send(
            bytes_data=self.gen_reply_bytes(
                ...
                defines.ChatType.SystemCenter.value,
                defines.SystemInfo,
                defines.MessageType.MessageNewUnRead,
